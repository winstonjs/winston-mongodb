...
let MongoDB = exports.MongoDB = function(options) {
  ...
  function createCollection(db) {
    ...
    return db.createCollection(self.collection, opts).then(col=>{
      ...
      return col.indexInformation({full: true}).then(indexInfos=>{
        indexInfos = indexInfos.filter(info=>info.name === ttlIndexName);
        if (indexInfos.length === 0) { // if its a new index then create it
          ...
          if (indexInfos[0].expireAfterSeconds !== self.expireAfterSeconds) {
            return col.dropIndex(ttlIndexName).then(()=>col.createIndex({timestamp: -1}, indexOpts))
            .then(()=>db);
          }
        }
      });
