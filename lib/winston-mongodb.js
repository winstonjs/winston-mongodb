 ...
 * @param {boolean=false} options.skipCollectionCreation Will disable the collection creation at startup.
 * @param {function} options.transformDocument A function that takes the default log document and transform it to another document.
 */
let MongoDB = exports.MongoDB = function(options) {
  ...
  this.skipCollectionCreation = options.skipCollectionCreation;
  this.transformDocument = options.transformDocument || function(d){return d;};
  this._opQueue = [];
  let self = this;
  ...
  function createCollection(db) {
    if (self.skipCollectionCreation) {
...
MongoDB.prototype.log = function(info, cb) {
  ...
  process.nextTick(()=>{
    ...
    entry = this.transformDocument(entry);
