...
let MongoDB = exports.MongoDB = function(options) {
  ...
  function createCollection(db) {
    ...
    return db.createCollection(self.collection, opts).catch(err=>{
      if (err.message !== 'collection already exists' || err.code !== 48) throw err; // swallow connection exists errors
